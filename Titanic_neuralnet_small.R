...
train.Embarked <- class.ind(as.factor(m3.titanic$Embarked))
colnames(train.Embarked)<- c("El1", "El2", "El3", "El4")
...
trainingset <- cbind(train.Pclass, train.Sex, train.Age, train.Embarked, train.Survived, train.SibSp, train.Parch)

#######smaller set#############
hold1<-trainingset[, 1:5]
hold2<-trainingset[, 30:31]
trainingset2<-cbind(hold1, hold2) #seeing if i can get this to work on a smaller set

test1<-cbind(test.Pclass, test.Sex, test.Survived)

head(test1)

## 1. Fit a Single Hidden Layer Neural Network using Least Squares
train.nnet<-nnet(Sl1~ Pcl1 + Pcl2 + Pcl3 + Xl1 +Xl2,trainingset2, size = 3, Hess=FALSE,decay=15e-4,maxit=250)

## Use TEST data for testing the trained model
test.nnet<-predict(train.nnet,test1)
new<-test.nnet
...
final_neuralnet<-cbind(titanictest$PassengerId, new)

write.csv(final_neuralnet, "C:/Users/julie.wisch/Documents/Kaggle_Titanic/SmallNeuralModelResults.csv")

#This also scores a 0.76555...it is no different in performance as compared to a simple regression
#Not super surprising, but I would anticipate that building complexity will increase the utility of a neural network
